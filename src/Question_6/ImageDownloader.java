...
public class ImageDownloader extends JFrame {
    ...
    private void removeTaskPanel(DownloadTaskPanel panelToRemove) {
        if (taskPanels.contains(panelToRemove)) {
            taskPanels.remove(panelToRemove);
            ...
            if (!canceled) {
                ...
                for (int i = 1; i <= totalChunks; i++) {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        appendStatus("Downloading: " + progress + "%");
                        updateProgressBar(progress);
                    });

                    Thread.sleep(500);  // Simulate chunk download time
                    if (canceled) {
                        SwingUtilities.invokeLater(() -> appendStatus("Download canceled: "));
...
Thread.sleep(100); // Sleep to avoid busy-wait
    ...
    private void saveImage(URL imageUrl) {
        try {
            ...
            String savedFileName = "downloaded_image_" + System.currentTimeMillis() + "." + extension;
            ...
            Path destination = imagePath.resolve(savedFileName);
